...
def test_model_by_onnxruntime(model):
    ...
    image = Image.open("D:/eyethink_project/network_split_software/example/input.jpg").convert('RGB')
    ...
    for node in conv_layers:
        ...
        if ort_outs[0] is not None:
            ...
            if(ort_outs_0_tensor.shape[1]<32):
                tensor_output(x = ort_outs_0_tensor, type='INT8', order=(0,2,1,3), filename=f"./data/conv{conv_layer_name}_input")
            else:
                tensor_output(x = ort_outs_0_tensor, type='INT8', order=(0,2,3,1), filename=f"./data/conv{conv_layer_name}_input")
        ...
        if ort_outs[1] is not None:
            ...
            tensor_output(x = ort_outs_1_tensor, type='INT8', order=(2,3,1,0), filename=f"./data/conv{conv_layer_name}_weight")
        ...
        if ort_outs[2] is not None:
            ...
            tensor_output(x = ort_outs_2_tensor, type='INT8', order=(0,), filename=f"./data/conv{conv_layer_name}_bias")
        ...
        if ort_outs[3] is not None:
            ...
            tensor_output(x = ort_outs_3_tensor, type='INT8', order=(0,2,3,1), filename=f"./data/conv{conv_layer_name}_output")
...
onnx_model = onnx.load("D:/eyethink_project/network_split_software/example/test_relu200_cle_bias.onnx")
